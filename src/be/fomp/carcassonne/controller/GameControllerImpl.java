...
public class GameControllerImpl implements GameController {
	...
	private Ruleset gameRuleset = new Ruleset();
	...
	public void doAddPlayer(Player player) throws ActionNotAllowedException {
		// check game state
		if (model.getPlayers().size() >= gameRuleset.MAX_PLAYERS)
			throw new ActionNotAllowedException("Maximum Player limit [" + gameRuleset.MAX_PLAYERS + "] reached\n");
	...
	/* GameControl Actions */
	public void doStartGame(String name) throws ActionNotAllowedException,
			TileFactoryException {
		// TODO do validation in separate class
		if (model.getState() != GameState.INIT)
			throw new ActionNotAllowedException("Invalid Game State: "
					+ model.getState().toString());
		if (model.getPlayers().size() > gameRuleset.MAX_PLAYERS)
			throw new ActionNotAllowedException("Maximum player limit exceeded");
		if (model.getPlayers().size() < gameRuleset.MIN_PLAYERS)
			throw new ActionNotAllowedException("Minimum player limit exceeded");

		// Initialize game variables
		TileDeck deck = TileDeckFactory.getInstance().buildDeck(gameRuleset.DECK_PATH);
		model.setDeck(deck);
		...
		model.setRound(0);
		Collections.sort(model.getPlayers(), gameRuleset.PLAYER_ORDER);
		for (Player p : model.getPlayers()) {
			p.setFollowers(gameRuleset.STARTING_HAND.get(FollowerType.FOLLOWER));
		}
	...
	public void doAddPlayer() throws ActionNotAllowedException {
		...
		if (model.getState() == GameState.RUNNING) {
			throw new ActionNotAllowedException(
					"Can not add players while the game is in progress");
		}
		doAddPlayer(new PlayerImpl());
		playerController.doShowView();
	}
	...
	public void doRemovePlayer() throws ActionNotAllowedException {
		...
		if (model.getPlayers().size() <= gameRuleset.MIN_PLAYERS)
			throw new ActionNotAllowedException("Minimum Player limit [" + gameRuleset.MIN_PLAYERS + "] reached\n");
		...
		if (index == model.getPlayers().size())
			if (model.getPlayers().size() > 0) // Safeguard for when MIN_PLAYERS
												// is set to 0
				index--;
			else
				doAddPlayer();
	...
	public boolean tryPlaceTile() {
		...
		try {
			TileUtils.connectTiles(model.getActiveTile(), top, bottom, left,
					right);
	...
	public void doPlaceFollower() throws ActionNotAllowedException {
		...
		if (player == null)
			throw new ActionNotAllowedException("No active player");
		if (player.getFollowers() <= 0)
			throw new ActionNotAllowedException("You have no more followers");
		...
		if (followerLoc < 0)
			throw new ActionNotAllowedException("No follower placed");

		Area followerArea = activeTile.getBorder()[followerLoc];

		if (followerArea.getZone().hasFollowers())
			throw new ActionNotAllowedException(
					"This zone does already contain followers");
	...
	public void doNextRound() throws ActionNotAllowedException {
		// TODO Fix this (need to pass here to handle endgame)
		// if(model.getState() != GameState.RUNNING) throw new
		// ActionNotAllowedException("Game has not started yet");
		Tile activeTile = model.getDeck().getTile();
		System.out.println("doNextRound");
		if (activeTile == null) {
	...
	public void doClickTile(int x, int y) throws ActionNotAllowedException {
		...
		y = gameRuleset.MAX_TILES_PER_COL - 1 - y;
		System.out.println("doClickTile x= " + x + " y= " + y);
		// Set X and Y to map coordinates (center = 0,0)
		x -= gameRuleset.MAX_TILES_PER_ROW / 2;
		y -= gameRuleset.MAX_TILES_PER_COL / 2;
	...
	@Override
	public AreaType doClickFollowerLocation(int location)
			throws ActionNotAllowedException {
		System.out.println("location= " + location);

		AreaType type = model.getActiveTile().getBorder()[location]
				.getAreaType();
		System.out.println("areatype= " + type);

		if (!type.equals(AreaType.CROSSING) && !type.equals(AreaType.NONE)
	...
	private Player getActivePlayer() {
		if (model != null && model.getPlayers() != null)
			return model.getPlayers().get(
					model.getRound() % model.getPlayers().size());
		return null;
	}
	...
	@Override
	public void setGameRuleset(Ruleset ruleset) {
		gameRuleset = ruleset;		
	}
	
	@Override
	public Ruleset getGameRuleset() {
		return gameRuleset;
	}
}
