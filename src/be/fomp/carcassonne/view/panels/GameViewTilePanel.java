...
/**
 * Represents a Tile from the game. In essence a panel with an image drawn over
 * it.
 * 
 * @author sven
 * 
...
public class GameViewTilePanel extends JPanel implements GameView {
	...
	private void drawTile(Graphics g) {
		...
		double rotation = model == null ? 0 : Math.toRadians(model
				.getRotation());
		double tx = 0;
		double ty = 0;
		...
		g2d.drawImage(tileImage, 0, 0, getWidth(), getHeight(), 0, 0,
	...
	private void drawFollower(Graphics g) {
		...
		if (model != null && model.getFollowerLocation() != null
				&& model.getFollowerLocation() >= 0) {
			int loc = model.getFollowerLocation();
			double xPos = ((double) this.getWidth() * ((loc == 0 || loc == 8) ? 0.3
			...
			double yPos = ((double) this.getWidth() * ((loc == 0 || loc == 2) ? 0.1
			...
			xPos -= 3;
			Ellipse2D.Double circleCenter = new Ellipse2D.Double(xPos, yPos, 5,
					8);
			Ellipse2D.Double circleHead = new Ellipse2D.Double(xPos, yPos - 5,
					5, 5);
			Ellipse2D.Double circleArm1 = new Ellipse2D.Double(xPos - 3, yPos,
					6, 3);
			Ellipse2D.Double circleArm2 = new Ellipse2D.Double(xPos + 3, yPos,
					6, 3);
			Ellipse2D.Double circleLeg1 = new Ellipse2D.Double(xPos - 1,
					yPos + 5, 2, 5);
			Ellipse2D.Double circleLeg2 = new Ellipse2D.Double(xPos + 3,
					yPos + 5, 2, 5);
	...
	private void drawFollowerLocations(Graphics g) {
		Graphics2D g2d = (Graphics2D) g;
		for (int loc = 0; loc <= 12; loc++) {
			...
			g2d.setColor(Color.getColor(model.getFollowerOwner()));
			xPos -= 3;
			Ellipse2D.Double circleCenter = new Ellipse2D.Double(xPos, yPos, 10,
					10);
			Ellipse2D.Double circleCenter2 = new Ellipse2D.Double(xPos, yPos, 8,
					8);
//			Ellipse2D.Double circleHead = new Ellipse2D.Double(xPos, yPos - 5,
//					5, 5);
//			Ellipse2D.Double circleArm1 = new Ellipse2D.Double(xPos - 3, yPos,
